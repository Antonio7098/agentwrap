...
func (r *Runtime) Capabilities(context.Context) (agentwrap.Capabilities, error) {
	return agentwrap.Capabilities{
		...
		Features: map[agentwrap.Capability]agentwrap.CapabilitySupport{
			...
			agentwrap.CapabilityPromptCacheAdvisory: {Supported: true, Detail: "validates and audits caller cache directives while OpenCode manages message-level provider caching"},
			agentwrap.CapabilityPromptCacheNative:   {Supported: false, Detail: "OpenCode CLI does not expose caller-defined cache routing keys or byte breakpoints"},
		},
		Unsupported: []agentwrap.UnsupportedCapability{
			...
			{Capability: agentwrap.CapabilityPromptCacheNative, Reason: "OpenCode CLI does not expose caller-defined cache routing keys or byte breakpoints"},
		},
	}, nil
}
